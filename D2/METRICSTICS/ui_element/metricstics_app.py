...
class MetricsticsApp(tk.Tk):
    def __init__(self):
        ...
        if authenticated:
            ...
            self.file_menu.add_command(
                label="Generate Data", command=self.generate_data
            )
            ...
            self.tool_menu.add_command(
                label="Load Pre-data", command=self.load_pre_data
            )
            ...
            self.left_frame.pack(
                side="left", fill="y", expand=True, padx=(10, 0), pady=10
            )
            self.main_frame.grid_columnconfigure(0, weight=1)

            self.middle_frame = MiddleFrame(
                self.main_frame, self, self.calculator, self.right_frame
            )
    ...
    def enter_data(self):
        try:
            ...
            messagebox.showerror(
                "Error", f"An error occurred while entering data: {str(e)}"
            )
    ...
    def generate_data(self):
        try:
            ...
            messagebox.showerror(
                "Error", f"An error occurred while generating data: {str(e)}"
            )
    ...
    def load_pre_data(self):
        ...
        listbox.bind(
            "<<ListboxSelect>>",
            lambda event: on_file_select(listbox.get(listbox.curselection())),
        )
    ...
    def export_data(self):
        ...
        if not results_data:
            tk.messagebox.showerror(
                "Export Error", "No results data available to export."
            )
            return
