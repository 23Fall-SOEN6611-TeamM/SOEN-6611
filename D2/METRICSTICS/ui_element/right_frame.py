    ...
    def draw_bar_chart(self, data, stats):
        def plot():
            ...
            if min_val in data:
                ...
                ax.annotate(
                    "m",
                    (x_values[data.index(min_val)], min_val),
            ...
            if max_val in data:
                ...
                ax.annotate(
                    "M",
                    (x_values[data.index(max_val)], max_val),
                    textcoords="offset points",
                    xytext=(0, 10),
                    ha="center",
                )
        ...
        def plot():
            ...
            def mark_point(value, label, x_offset=0):
                if value is not None:
                    closest_index = min(
                        range(len(data)), key=lambda i: abs(data[i] - value)
                    )
                    ax.plot(closest_index + 1, data[closest_index], "ro")
                    ax.annotate(
                        label,
                        (closest_index + 1, data[closest_index]),
                        textcoords="offset points",
                        xytext=(x_offset, 10),
                        ha="center",
                    )
        ...
        def plot():
            ...
            def mark_point(value, label, y_offset=10, x_offset=0):
                if value is not None:
                    ...
                    ax.annotate(
                        label,
                        (closest_index, data[closest_index]),
                        textcoords="offset points",
